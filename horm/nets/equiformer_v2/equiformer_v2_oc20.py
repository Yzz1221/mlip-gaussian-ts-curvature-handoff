...
class EquiformerV2_OC20(BaseModel):
    ...
    def __init__(
        ...
        # 无 GPU / 无驱动时勿调用 torch.cuda.current_device()，否则会在 __init__ 里触发 CUDA 初始化并报错
    ...
    def forward(self, data):
        ...
        # 不在 forward 内算 grad_hess_ij：返回值未使用，且会触发 autograd 错误；
        # Gaussian external 的 Hessian 由 eval.compute_hessian 对 pos/energy/force 另行计算。
        ###############################################################
        # Force estimation
        ###############################################################
